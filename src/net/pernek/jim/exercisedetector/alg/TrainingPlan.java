...
public class TrainingPlan {
	...
	public static TrainingPlan parseFromJson(String jsonString) throws JSONException{
		JSONObject jTrainingPlan = new JSONObject(jsonString);
		
		TrainingPlan retVal = TrainingPlan.create(jTrainingPlan.getString("name"));
		...
		for(int i=0; i < jExercises.length(); i++){
			...
			Exercise newExercise = Exercise.create(jCurrentExercise.getString("name"));
			for(int j=0; j < jSeries.length(); j++){
				JSONObject jCurrentSeries = jSeries.getJSONObject(j);
				Series newSeries = Series.create(jCurrentSeries.getInt("repeat_count"), jCurrentSeries.getInt("weight"));
